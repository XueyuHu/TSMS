...
def create_vector(first_elementA="K", first_elementa = "La", second_element="Cr", third_element="Al", vector_size=32, split_firstA=8, split_firsta = 16, split_second=24, num_replacements=0):
    """Create a vector with specified elements and replacements."""
    vector = np.empty(vector_size, dtype=object)  # Create an empty vector
    vector[:split_firstA] = first_elementA  # Fill the first part with the first element
    vector[split_firstA:split_firsta] = first_elementa
    vector[split_firsta:split_second] = second_element  # Fill the middle part with the second element
...
def create_vector2(first_elementA="K", first_elementa = "La", second_element="Cr", third_element="Al", fourth_element="As", vector_size=32, split_firstA=8, split_firsta = 16, split_second=24, num_replacements=0, num_replacements2=1):
    """Create a vector with specified elements and replacements."""
    vector = np.empty(vector_size, dtype=object)  # Create an empty vector
    vector[:split_firstA] = first_elementA  # Fill the first part with the first element
    vector[split_firstA:split_firsta] = first_elementa
...
def Enlargement(vector):
    ...
    Ratio_1 = 0
    Ratio_2 = 0
    Ratio_3 = 1  # double with 1 and single with 0
    ...
    for element, count in element_counts.items():
        ...
        if element in Aa:
            radius_dict['radius_3'] += getattr(data(), element)['radius12'] * count
        if element in B1:
    ...
    vector.append(radius_dict['radius_3'] / counts_A)
    ...
    vector.append((radius_dict['radius_3'] / counts_A) / (radius_dict['radius_1'] / counts_A))
    #vector.append(0)
    vector.append((radius_dict['radius_4'] / (counts_B1 + counts_B2)) / (radius_dict['radius_2'] / (counts_B1 + counts_B2)))
    vector.append(((radius_dict['radius_3'] / counts_A) + (radius_dict['radius_4'] / (counts_B1 + counts_B2))) / (
    ...
    for element, count in element_counts.items():
        for i in variables:
            ...
            if element in Aa:
                variables[i][3] += getattr(data(), element)[i] * count  # 就这样吧 无所谓了
            if element in B1:
    ...
    for i in variables:
        ...
        vector.append(variables[i][3] / counts_A)
        ...
        vector.append((variables[i][3] / counts_A) / (variables[i][1] / counts_A))
        vector.append((variables[i][4] / (counts_B1 + counts_B2)) / (variables[i][2] / (counts_B1 + counts_B2)))
        vector.append(((variables[i][3] / counts_A) + (variables[i][4] / (counts_B1 + counts_B2))) / (
...
# Configuration
first_elements = ["K","Cs","Ba","Na","Mg","Ca","Sr","La","Pr","Nd","Sm"]

AA = ["Mg","Ca","Sr"]#["K","Rb","Cs","Ba","Na","Mg","Ca","Sr"]
Aa = ["La","Pr","Nd","Sm"]
        ...
        for second_element in tqdm(second_elements):
            ...
            for third_element in tqdm(third_elements):
                for num_replacements in range(1, 9):  # From 1 to 8 replacements in the last 8 positions
                    vector = create_vector(first_elementA, first_elementa, second_element, third_element, vector_size, split_firstA, split_firsta, split_second,
                                                   num_replacements)
                    #print(f"Vector with {num_replacements} replacement(s) by '{third_element}':")
                    List_Vector = vector.tolist()
                    cost = Cost(List_Vector)
                    with open('%s4_cost.csv' % first_elementA, 'a', newline='') as file:
                        writer = csv.writer(file)
                    ...
                    if num_replacements >= 2:
                        for fourth_element in third_elements[index3:]:
                            ...
                            for num_replacements2 in range(1, num_replacements):
                                vector = create_vector2(first_elementA, first_elementa, second_element, third_element, fourth_element, vector_size, split_firstA, split_firsta, split_second, num_replacements, num_replacements2)
                                List_Vector = vector.tolist()
                                cost = Cost(List_Vector)
                                with open('%s4_cost.csv' % first_elementA, 'a', newline='') as file:
                                    writer = csv.writer(file)
