#########################
# Xueyu Hu, March 28th, Generating input information of First-stage Machine Learning
import os
import pandas as pd
import csv
class data():
	...
	Mg={'radius6':0.72,'radius12':0,'electronAffinity':-0.1,'oxidationstate':2,'meltingpointSS':923,'boilingpointSS':1363,'atomicMass':24.305,'density':1.738,'ionizationEnergy':738,'electronnegativity':1.31,'meltingpointO':2825,'chemicalPotential':-507.672}
...
A = ["Na","K","Rb","Cs","Sr","Ba","Pr"]
B1 = ["Cr","Mn","Fe","Co","Ni","Cu"]
B2 = ["Al","As","Bi","Ca","Ce","Dy","Er","Eu","Ga","Gd","Ge","Hf","Ho","In","La","Lu","Mg","Mo","Nb","Nd","Pb","Sb","Sc","Sm","Sn","Ta","Tb","Te","Ti","Tm","V","W","Y","Yb","Zn","Zr"]#['Al','As','Bi','Ca','Cr','Cr','Dy','Er','Eu','Ga','Gd','Ge','Hf','Ho','In','La','Lu','Mg','Mo','Nb','Nd','Pb','Sb','Sc','Sm','Sn','Ta','Tb','Te','Ti','Tm','V','W','Y','Yb','Zn','Zr']
print(vector)
element_counts = {}
...
for element,count in element_counts.items():
	if element in A:
		radius_1 += getattr(data(),element)['radius12']*count #需要除一下
		radius_5 += getattr(data(),element)['radius12']*count #就这样吧 无所谓了
	if element in B1:
		radius_2 += getattr(data(),element)['radius6']*count
		radius_6 += getattr(data(),element)['radius6']*count
	if element in B2:
		radius_2 += getattr(data(),element)['radius6']*count
		radius_4 += getattr(data(),element)['radius6']*count
vector.append(radius_1/counts_A)
vector.append(radius_2/(counts_B1+counts_B2))
vector.append(radius_3)
vector.append(radius_4/(counts_B1+counts_B2))
vector.append(radius_5/counts_A)
vector.append(radius_6/(counts_B1+counts_B2))
vector.append(radius_5/counts_A + radius_6/(counts_B1+counts_B2)) #7
vector.append((radius_1/counts_A)/(radius_2/(counts_B1+counts_B2)))
vector.append((radius_5/counts_A)/(radius_6/(counts_B1+counts_B2)))
vector.append(radius_3/(radius_1/counts_A))
vector.append((radius_4/(counts_B1+counts_B2))/(radius_2/(counts_B1+counts_B2)))
vector.append((radius_3+(radius_4/(counts_B1+counts_B2)))/((radius_5/counts_A)+(radius_6/(counts_B1+counts_B2))))
#for i in range(1,13):
...
for i in variables:
	...
	vector.append((variables[i][3]+(variables[i][4]/(counts_B1+counts_B2)))/((variables[i][5]/counts_A)+(variables[i][6]/(counts_B1+counts_B2))))

vector.append((radius_1/counts_A+1.4)/((radius_2/(counts_B1+counts_B2)+1.4)*2**0.5)) #TF
vector.append(radius_2/(counts_B1+counts_B2)/1.4) #OF
vector.append((variables['atomicMass'][1]/counts_A+variables['atomicMass'][2]/(counts_B1+counts_B2)+getattr(data(),'O')['atomicMass']*3)*((radius_1/counts_A)*(radius_2/(counts_B1+counts_B2)))**-0.5) #density
vector.append((6-variables['oxidationstate'][1]/counts_A-variables['oxidationstate'][2]/(counts_B1+counts_B2))/2)
vector.append(abs(((radius_1/counts_A+1.4)/((radius_2/(counts_B1+counts_B2)+1.4)*2**0.5))-1))
